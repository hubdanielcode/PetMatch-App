    ...
    <div className="w-full px-4 sm:px-6 pb-6">
      ...
      <div className="-mt-6 bg-white dark:bg-gray-200 rounded-2xl shadow-lg shadow-gray-500 px-4 sm:px-5 py-4 mb-6 flex items-center gap-3">
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-orange-100 text-orange-600 flex items-center justify-center shrink-0 rounded-lg">
          <FaMars className="w-4 h-4 sm:w-5 sm:h-5" />
        </div>
        <div className="min-w-0">
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-800 font-medium mb-1">
            Etapa 6
          </p>
          <p className="text-lg sm:text-xl font-bold text-black">Testículos</p>
        </div>
      </div>
      ...
      <div className="-mb-7">
        {TesticleOptions.map((option) => {
          ...
          return (
            <button
              className={`group flex min-h-10 w-full cursor-pointer items-center gap-4 rounded-lg bg-gray-200 px-4 py-3 text-start transition-all mb-4 ${
                isSelected
                  ? "border border-amber-600 bg-linear-to-br from-amber-200 via-orange-200 to-red-200 hover:bg-linear-to-br hover:from-amber-100 dark:hover:from-amber-400 hover:via-orange-100 dark:hover:via-orange-400 hover:to-red-100 dark:hover:to-red-400"
                  : "border border-black/40 hover:bg-amber-50 dark:hover:bg-gray-600"
              }`}
              key={option.value}
              type="button"
              onClick={() => {
            ...
            >
              ...
              <div className="flex-1">
                <p
                  className={`text-md font-semibold text-black transition-colors ${
                    !isSelected && "dark:group-hover:text-white"
                  }`}
                >
                  {option.label}
                </p>
                <p
                  className={`text-sm text-black/70 transition-colors ${
                    !isSelected && "dark:group-hover:text-white/70"
                  }`}
                >
                  {option.description}
                </p>
